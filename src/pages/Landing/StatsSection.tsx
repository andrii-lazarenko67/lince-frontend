...
const stats: StatItem[] = [
  {
    ...
    suffix: '+',
    label: 'Companies',
    description: 'Water treatment companies using LINCE',
  ...
  },
  {
    ...
    suffix: '+',
    label: 'Inspections',
    description: 'Field inspections logged on the platform',
  ...
  },
  {
    ...
    suffix: '+',
    label: 'Records',
    description: 'Daily monitoring records stored securely',
  ...
  },
  {
    ...
    suffix: '%',
    label: 'Uptime',
    description: 'Platform reliability and availability SLA',
...
const StatCard: React.FC<{ stat: StatItem; isVisible: boolean; delay: string }> = ({ stat, isVisible, delay }) => {
  ...
  return (
    <div className={`relative text-center group anim-fade-up ${delay} ${isVisible ? 'anim-visible' : ''}`}>
      ...
      {/* Label */}
      <div className="text-base font-semibold text-slate-200 mb-1.5">{stat.label}</div>
      <div className="text-slate-500 text-sm leading-relaxed max-w-[180px] mx-auto">{stat.description}</div>
    </div>
  );
};
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-6 divide-y-0">
          {stats.map((stat, i) => (
            <StatCard key={stat.label} stat={stat} isVisible={isVisible} delay={delays[i]} />
          ))}
        </div>
      </div>
    </section>
