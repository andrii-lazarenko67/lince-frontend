  ...
  return (
    <div className="w-full">
          ...
          return (
            ...
            >
              <div className="flex justify-between text-xs">
                ...
                <span className="text-gray-500">{total} {t('charts.items')}</span>
              </div>
              ...
              {hoveredIndex === index && (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    ...
                    {t('charts.pass')}: <span className="text-green-400 font-semibold">{item.pass} ({passPercent.toFixed(0)}%)</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    <div className="w-2.5 h-2.5 rounded bg-red-500" />
                    {t('charts.fail')}: <span className="text-red-400 font-semibold">{item.fail} ({failPercent.toFixed(0)}%)</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    <div className="w-2.5 h-2.5 rounded bg-gray-400" />
                    {t('charts.na')}: <span className="text-gray-300 font-semibold">{item.na} ({naPercent.toFixed(0)}%)</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="flex justify-center gap-4 mt-4">
        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-green-500" /><span className="text-xs text-gray-600">{t('charts.pass')}</span></div>
        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-red-500" /><span className="text-xs text-gray-600">{t('charts.fail')}</span></div>
        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-gray-400" /><span className="text-xs text-gray-600">{t('charts.na')}</span></div>
      </div>
    </div>
  );
};
